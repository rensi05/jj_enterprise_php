...
<section class="content-header">
    <div class="container-fluid">
        ...
        <div class="text-right">
            <a class="" href="{{route('addunit')}}" >
                <button type="button" class="btn bg-gradient-primary" >Add Units</button>
            </a>
        </div>
    </div><!-- /.container-fluid -->
</section>
...
<script>
            ...
            "columns": [
                ...
                {"taregts": 3, 'data': 'status', "searchable": false, "orderable": false,
                    "render": function (data, type, row) {
                        ...
                        var out = '';
                        if (status == "active") {
                            out += '<button title="Active" class="btn btn-success waves-effect waves-light" onClick="changeunitStatusModal(' + id + ')">Active</button>&nbsp;&nbsp;&nbsp;';
                        } else {
                            out += '<button title="Inactive" class="btn btn-danger waves-effect waves-light" onClick="changeunitStatusModal(' + id + ')">Inactive</button>';
                        }
                        return out;
                    }
                },
                {"taregts": 4, "searchable": false, "orderable": false,
                    "render": function (data, type, row) {
                        var id = row.id;
                        var out = '';
                        out += '<a title="Edit " href="{{url("editunit")}}/' + id + '" class="btn btn-primary waves-effect waves-light">Edit</a>&nbsp;&nbsp;&nbsp;';
                        out += '<button title="Delete Unit" class="btn btn-danger waves-effect waves-light" onClick="deleteUnit(' + id + ');">Delete</button>';
                        return out;
                    }
                },
    ...
    function changeunitStatusModal(id) {
        $.ajax({
            url: user_path + 'changeunitstatus/' + id,
    ...
    function deleteUnit(id) {
        $.ajax({
            url: user_path + 'unitdeletemodal/' + id,
            success: function (response) {
                if (response.status == 'success') {
